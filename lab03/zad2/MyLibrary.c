...
void writeResult(clock_t start, clock_t end, struct tms* t_start, struct tms* t_end, char * fileA, char* fileB)
{
...
int combine2FILES(FILE *A, FILE *B) 
{
    ...
    int sizeOfFile=0;
    char name[30];
    sprintf(name, "merge%d.txt", getpid());
    FILE *generated = fopen(name,"w"); //tworzenie pliku tymczasowego
...
int combineFILES(char **args, int lenght, struct MainArray* mainArr)
{
    ...
    for(i=0;i<lenght;i++)
    {
        ...
        //we can implement multiple processes here
         ...
         if(parent== getpid())
        {
            ...
            else 
            {
                ...
                writeResult(start, end, &tmsStart, &tmsEnd, files[0], files[1]);
                sleep(5);
